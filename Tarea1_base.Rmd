...
El nivel es el hogar, para eso está el household_id, pero queremos ver si es un panel balanceado o no.
Hay dos maneras de revisarlo, 1) generaremos una variable que sea el número de compra de cada hogar (household ID), para esto usamos **ave()** con la función **seq_along**. 2) Otra manera de hacerlo es con dplyr. Comprobamos que ambos resultados nos dan el mismo resultado.
